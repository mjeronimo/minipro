								...
								void *user_data)
{
	...
	if (opcode == BT_ATT_OP_HANDLE_VAL_NOT) {
		printf("notify_cb: Received Value Notification (BT_ATT_OP_HANDLE_VAL_NOT)\n");
		util_hexdump('<', pdu, length, (util_debug_func_t) printf, NULL);
		printf("\n");
		fflush(stdout);
	}
